# Hi, I'm Aarti Wamane

**M.Tech Computer Engineering | Full-Stack Developer**

## 🤖 Machine Learning 

### 📊 ML Case Studies & Implementations

* [Manual KNN Classifier](https://github.com/aartiwamane/Manual-KNN-Classifier)
* [Linear Regression From Scratch](https://github.com/aartiwamane/Linear-Regression-From-Scratch-Python)
* [Breast Cancer Classification](https://github.com/aartiwamane/BreastCancer-LogisticRegression)
* [Loan Default Prediction](https://github.com/aartiwamane/MLP-Loan-Default-Prediction)
* [Fraudulent Transaction Detection](https://github.com/aartiwamane/Fraudulent-Transaction-Detection-ML)


## 🐍 Python & Automation & Utilities

* [Python Empty File Cleaner](https://github.com/aartiwamane/Python-Empty-File-Cleaner)
* [Python System Surveillance Automation](https://github.com/aartiwamane/Python-System-Surveillance-Automation)
* [Python SMTP Mail Sender](https://github.com/aartiwamane/python-smtp-mail-sender)
* [Python Custom Filter Map Reduce](https://github.com/aartiwamane/Python-Custom-Filter-Map-Reduce)
* [Duplicate File Detector]([PASTE-REPOSITORY-LINK](https://github.com/aartiwamane/Duplicate-File-Removal-Automation))

## 🌐 Angular & Frontend Development
### 🚀 Angular Applications

* [EventHub](https://github.com/aartiwamane/Event-Hub-)
* [Angular CRUD Application](https://github.com/aartiwamane/angular-crud-app)
* [Angular JSON Server](https://github.com/aartiwamane/Angular-JSON-Server)

  ### 🧩 Angular Concept Implementations

* [Child to Parent Component Communication](https://github.com/aartiwamane/angular-child-to-parent-output)
* [Parent to Child Communication](https://github.com/aartiwamane/angular-parent-to-child-communication)
* [Multiple Service Injection](https://github.com/aartiwamane/angular-multi-service-injection)
* [FormControl Setup with Bootstrap](https://github.com/aartiwamane/angular-formcontrol-setup-bootstrap)

## 🛠️ Technologies

**Programming:** Python, C, C++, Java

**Machine Learning:** NumPy, Pandas, Scikit-learn, Matplotlib

**Frontend:** Angular, TypeScript, JavaScript, HTML, CSS, Bootstrap

**Backend:** Node.js, Express.js, REST APIs

**Database:** MongoDB, JSON Server
